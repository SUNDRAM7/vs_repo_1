...
OUTPUT_FOLDER = r"C:\Users\Axxela\Desktop\atm strategy final\matrix\t90\sl150_t90"
FIXED_TARGET = 0.05
...
class OptionTracker:
    ...
    def add_straddle(self, call_strike, put_strike, entry_time, call_bid, put_bid):
        ...
        call_sl = 1.25*(float(min(call_bid * 2, call_bid + put_bid)))#stoploss ke ticks ka 50%
        put_sl =  1.25*(float(min(put_bid * 2, call_bid + put_bid)))
    ...
    def update_positions(self, current_time, bid_cache):
        for call_pos, put_pos in self.pairs:
            ...
            if both_open and call_ask and put_ask:
                total_pnl = (call_pos["entry_bid"] - call_ask) + (put_pos["entry_bid"] - put_ask)
                avg_entry = (call_pos["entry_bid"] + put_pos["entry_bid"]) * 0.9
