...
# === CONFIGURATION === #
CSV_FOLDER = r"C:\Users\Axxela\Desktop\spx_tzconverted_220704to 250701"
OUTPUT_FOLDER = r"C:\Users\Axxela\Desktop\atm strategy final\test11"
FIXED_TARGET = 0.05
...
class OptionTracker:
    ...
    def add_straddle(self, call_strike, put_strike, entry_time, call_bid, put_bid,call_high, call_low, put_high, put_low):
        #print(8)
    ...
    def update_positions(self, current_time, bid_cache):
        ...
        for call_pos, put_pos in self.pairs:
            ...
            call_high = get_cached_price(call_pos["symbol"], current_time, bid_cache, "high")
            put_high = get_cached_price(put_pos["symbol"], current_time, bid_cache, "high") 
            call_low = get_cached_price(call_pos["symbol"], current_time, bid_cache, "low")
            put_low = get_cached_price(put_pos["symbol"], current_time, bid_cache, "low")  
                    

            if both_open and call_ask and put_ask:
                total_pnl = (call_pos["entry_bid"] - call_low) + (put_pos["entry_bid"] - put_low)
                avg_entry = (call_pos["entry_bid"] + put_pos["entry_bid"]) *0.95

                if total_pnl >= avg_entry:
                    self._close(call_pos, current_time, call_ask, "TARGET HIT",call_low)
                    self._close(put_pos, current_time, put_ask, "TARGET HIT",put_low)
                    continue
            ...
            for pos in [call_pos, put_pos]:
                ...
                call_high= get_cached_price(pos["symbol"], current_time, bid_cache, "high")
                put_high =get_cached_price(pos["symbol"], current_time, bid_cache, "high")
                #print(call_high,put_high)
                ...
                '''if pos["target"] is not None and ask is not None and ask <= pos["target"]:
                    self._close(pos, current_time, ask, "TARGET HIT (REVISED)",0.05)'''
                if pos["target"] == FIXED_TARGET and ask is not None and ask <= FIXED_TARGET:
                    self._close(pos, current_time, ask, "TARGET HIT (POST SL)",FIXED_TARGET)
...
def preload_price_data(date_str, symbols):#working
    ...
    query = f"""
        SELECT symbol, ts_recv, bid_px_00, ask_px_00, high, low  FROM {table_name}
...
def process_file(csv_file_path):
    ...
    for _, row in df.iterrows():
        ...
        call_low = get_cached_price(call_symbol, time_now, price_cache, "low")
        put_high =get_cached_price(put_symbol, time_now, price_cache, "high")
        put_low = get_cached_price(put_symbol, time_now, price_cache, "low")
        #print(call_high,put_low)

        if (call_bid and call_bid > FIXED_TARGET) and (put_bid and put_bid > FIXED_TARGET):
            tracker.add_straddle(call_strike, put_strike, time_now, call_bid, put_bid, call_high, call_low, put_high, put_low)
