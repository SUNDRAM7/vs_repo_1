...
def get_matching_tables(cursor):
    ...
    source_tables = [row[0] for row in cursor.fetchall() if row[0].startswith('ohlc_exp_')]

    suffixes = [tbl.replace('ohlc_exp_', '') for tbl in source_tables]
    matching = []

    for suffix in suffixes:
        src = f"ohlc_exp_{suffix}"
...
def update_in_batches(cnx, src_table, tgt_table):
    ...
    count_query = f"""
        ...
        ON b.symbol = o.symbol AND b.ts_recv = o.ts_event
    """
