...
OUTPUT_FOLDER = r"C:\Users\Axxela\Desktop\atm strategy final\trade_matrix"
FIXED_TARGET = 0.05
...
class OptionTracker:
    ...
    def update_positions(self, current_time, bid_cache):
        ...
        for call_pos, put_pos in self.pairs:
            ...
            call_close = get_cached_price(call_pos["symbol"], current_time, bid_cache, "close")
            put_close = get_cached_price(put_pos["symbol"], current_time, bid_cache, "close")  
                    

            if both_open and call_ask and put_ask:
                total_pnl = (call_pos["entry_bid"] - call_close) + (put_pos["entry_bid"] - put_close)
                avg_entry = (call_pos["entry_bid"] + put_pos["entry_bid"]) *self.tp

                if total_pnl >= avg_entry:
                    self._close(call_pos, current_time, call_close, "TARGET HIT",call_close)
                    self._close(put_pos, current_time, put_close, "TARGET HIT",put_close)
                    continue
...
def preload_price_data(date_str, symbols):#working
    ...
    query = f"""
        SELECT symbol, ts_recv, bid_px_00, ask_px_00, high, close  FROM {table_name}
